...
(define (problem locality-02)
  ...
  (:objects
   ...
   ;15 data blocks to be scattered.
    ...
    count-0 - zero
    count-1 count-2 count-3 count-4 count-5 count-6 count-7 count-8 count-9 count-10 count-11 count-12 count-13 count-14 count-15 - non-zero
)
(:init
    (= (total-cost) 0)
    (not (has-requested node-E))
    (not (has-requested node-F))
    (not (has-requested node-G))
    (not (has-requested node-H))
    (in block-0 node-F)
    (in block-1 node-F)
    (in block-2 node-F)
    (in block-3 node-E)
